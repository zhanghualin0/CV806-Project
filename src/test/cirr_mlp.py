...
class TestCirr:
    ...
    def __call__(model, data_loader, fabric):
        ...
        for ref_img, tar_feat, caption, pair_id, *_ in data_loader:
            ...
            # concatenated_feats = torch.cat((query_feat.unsqueeze(1), image_feat.unsqueeze(1), text_feat.unsqueeze(1)), dim=1)
            concatenated_feats = torch.cat([query_feat, image_feat, text_feat], dim=-1)
            concatenated_feats = concatenated_feats.view(concatenated_feats.size(0), -1)  # Flatten for MLP
            
            # Obtain weights from MLP
            weights = model.weighting_mlp(concatenated_feats)
            
            # Apply weights
            weighted_feat = weights[:, 0].unsqueeze(1) * query_feat + \
                            weights[:, 1].unsqueeze(1) * image_feat + \
                            weights[:, 2].unsqueeze(1) * text_feat
            weighted_feat = F.normalize(weighted_feat, dim=-1)

            # Use weighted_feat instead of query_feat for further processes
            query_feats.append(weighted_feat.cpu())
